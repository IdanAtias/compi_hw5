...
// Our Matrix operations NOT TESTED!!

template<typename T>
QSMatrix<T> QSMatrix<T>::power(int i) {
	if (i <= 0) {
		return *this;
	}
	QSMatrix result = *this;
...
template<typename T>
QSMatrix<T> QSMatrix<T>::concat(const QSMatrix<T>& rhs) {
	unsigned rhsCols = rhs.get_cols();
	unsigned newCols = rhs.get_cols() + this->cols;
	QSMatrix<T> result(rows, newCols, 0.0);
	for (unsigned i = 0; i < rows; i++) {
		for (unsigned j = 0; j < cols; j++) {
			result(i, j) = (*this)(i, j);
		}
...
template<typename T>
const T& QSMatrix<T>::operator()(const unsigned& row,
...
template<typename T>
void QSMatrix<T>::printMatrix() const {
	for (unsigned i = 0 ; i < rows ; ++i) {
		for (unsigned j = 0 ; j < cols ; ++j) {
			std::cout << (*this)(i,j);
			if (j < (cols - 1)) {
